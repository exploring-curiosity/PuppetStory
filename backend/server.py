...
async def _dispatch_text_tag(ws, _ts_fn, tag_name: str, raw_attrs: str):
    import html
    ...
    if tag_name == "set_scene":
        ...
        if puppets_match:
            try:
                ...
                pass
        scene_data = {"background_id": bg_id, "mood": mood, "transition": "crossfade", "puppets": puppets}
        print(f"[{_ts_fn()}] text-tag set_scene: bg={bg_id}, mood={mood}, puppets={len(puppets)}")
        ...
        print(f"[{_ts_fn()}] text-tag action_sequence: dur={duration}s, puppets={list(puppets_kf.keys())}")
...
async def story_websocket(ws: WebSocket):
    ...
    async def on_narration_text(text: str):
        ...
        for tag_name in ("set_scene", "action_sequence"):
            ...
            if match:
                ...
                try:
                    await _dispatch_text_tag(ws, _ts, tag_name, raw_attrs)
                except Exception as e:
    ...
    _pending_child_text = [None]
    _child_tool_nudge_sent = [False]
    _state = {"current_scene": None}

    try:
        await live_session.run(
            on_audio=_on_audio, on_tool_call=lambda tc: _handle_tool_call(tc, ws, live_session, _ts, assets, char_scales, _state),
            on_transcript=_on_transcript, on_ready=lambda live: _on_ready(live, ws, _live_ref, _ts, story),
            on_narration_text=lambda t: _on_narration_text(ws, _narration_buffer, _ts, _dispatch_text_tag, t),
            on_turn_complete=lambda: _on_turn_complete(ws, _live_ref, _last_auto_continue_t, _pending_child_text, _child_tool_nudge_sent, _ts),
        )
    except Exception as e:
...
async def _on_turn_complete(ws, _live_ref, _last_auto_continue_t, _pending_child_text, _child_tool_nudge_sent, _ts_fn):
    try:
        await ws.send_json({"type": "turn_complete"})
    except Exception:
        pass
    now = time.time()
    if _live_ref[0] is None or (now - _last_auto_continue_t[0]) <= 5.0:
        return
    _last_auto_continue_t[0] = now
    if _pending_child_text[0] and not _child_tool_nudge_sent[0]:
        _child_tool_nudge_sent[0] = True
        child_req = _pending_child_text[0][:80]
        print(f"[{_ts_fn()}] Tool-call nudge for child input: {child_req}")
        await _live_ref[0].send_text(
        ...
        print(f"[{_ts_fn()}] Auto-continue after TURN_COMPLETE")
        await _live_ref[0].send_text("Continue. Next beat — call set_scene and action_sequence. Keep narrating!")


async def _on_ready(live, ws, _live_ref, _ts_fn, story):
    _live_ref[0] = live
    print(f"[{_ts_fn()}] Gemini session connected, sending kickstart prompt")
    story_title = story["title"] if story else "a story"
    await live.send_text(f'Begin "{story_title}" now. Call set_scene first, then greet the child. Go!')
    asyncio.create_task(_browser_message_loop(ws, live, _ts_fn))


async def _puppet_inference_task(ws, _ts_fn, story, _state, child_text: str):
    try:
        ...
        if result.get("needs_scene_change") and result.get("set_scene"):
            sc = result["set_scene"]
            scene_data = {"background_id": sc.get("background_id", "meadow"), "mood": sc.get("mood", "cheerful"), "transition": "crossfade", "puppets": sc.get("puppets", [])}
            _state["current_scene"] = scene_data
            print(f"[{_ts_fn()}] PuppetInference set_scene: bg={scene_data['background_id']}, mood={scene_data['mood']}")
            await ws.send_json({"type": "set_scene", "data": scene_data})
        if result.get("action_sequence"):
            ...
            if puppets_kf:
                action_data = {"duration": duration, "puppets": puppets_kf}
                print(f"[{_ts_fn()}] PuppetInference action_sequence: dur={duration}s, puppets={list(puppets_kf.keys())}")
                await ws.send_json({"type": "action_sequence", "data": action_data})
    except Exception as e:
        print(f"[{_ts_fn()}] PuppetInference task error: {e}")


async def _browser_message_loop(ws, live, _ts_fn):
    try:
        while True:
                ...
                if "bytes" in message and message["bytes"]:
                    ...
                    try:
                        ...
                        if msg_type == "wind_down":
                            await live.send_realtime_text("[PARENT SIGNAL: sleepy time] — Begin winding down. Slow your pace, use calming imagery, bring the story to a peaceful conclusion.")
                        elif msg_type == "user_text":
                            text = data.get("text", "")
                            await ws.send_json({"type": "thinking", "text": text[:50]})
                            await live.send_realtime_text(text)
                            asyncio.create_task(_puppet_inference_task(ws, _ts_fn, story, _state, text))
                    except json.JSONDecodeError:
                        pass
